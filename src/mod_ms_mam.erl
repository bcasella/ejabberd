...
%%% Changed : 17 August 2015 by Takao <takao.matsumura@mstech.com.br>
%%%
...
%% Mstech - modified/customized the original do-store function
%% added 2 fields: from_user and to_user
%% Use mstech_mysql.sql to create database table containing these fields
do_store(Pkt, LUser, LServer, Peer, _Type, _Dir, odbc) ->
    ...
    FromUserXml = xml:get_attr_s(<<"from">>, Pkt#xmlel.attrs),
    ToUserXml = xml:get_attr_s(<<"to">>, Pkt#xmlel.attrs),

%%  TODO: Mstech - handle Peers (from/to) to remove resource properly
    [BareFromL|_FromRsrc] = string:tokens(bitstring_to_list(FromUserXml),"/"),
    BareFrom = list_to_bitstring(BareFromL),
    [BareToL|_ToRsrc] = string:tokens(bitstring_to_list(ToUserXml),"/"),
    BareTo = list_to_bitstring(BareToL),
    case ejabberd_odbc:sql_query(
           LServer,
           [<<"insert into archive (username, timestamp, "
              "peer, bare_peer, xml, from_user, to_user, txt) values (">>,
            <<"'">>, ejabberd_odbc:escape(LUser), <<"', ">>,
            <<"'">>, TS, <<"', ">>,
            <<"'">>, ejabberd_odbc:escape(LPeer), <<"', ">>,
            <<"'">>, ejabberd_odbc:escape(BarePeer), <<"', ">>,
            <<"'">>, ejabberd_odbc:escape(XML), <<"', ">>,
            <<"'">>, ejabberd_odbc:escape(BareFrom), <<"', ">>,
            <<"'">>, ejabberd_odbc:escape(BareTo), <<"', ">>,
